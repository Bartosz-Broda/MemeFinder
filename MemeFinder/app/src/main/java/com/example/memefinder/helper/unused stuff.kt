...
//imageList.remove(imageList[selectedPosition])
/*imageList.removeAt(selectedPosition)
galleryPagerAdapter.notifyDataSetChanged()

val cores = Runtime.getRuntime().availableProcessors()
var listNr = id % cores
var tmpList = activity?.let { it1 -> readListFromPref(it1, "${R.string.preference_file_key} $listNr") }
if (tmpList != null) {
    for (item in tmpList){
        if (item.uri == mUri){
            tmpList.remove(item)
    ...
    activity?.let { it1 -> writeListToPref(it1, tmpList, "${R.string.preference_file_key} $listNr") }
}
activity?.let { it1 -> writeListToPref(it1, imageList, R.string.preference_file_key.toString()) }
activity?.let { it1 -> writeListToPref(it1, imageList, imageListKey) }*/